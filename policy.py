            ...
            for i in range(self.n):
                ...
                for j in range(self.n):
                    ...
                    for _ in range(fm[i,j]):
                        ...
                        count += 1
                        grids[ix][iy].orders.append(Order(t_now, 1, ix, iy, jx, jy, f_p=2))
        ...
        for i in grids:
            for j in i:
                ...
                if future:
                    ...
                    # need add new orders
                # car left next t
                n_car[count] += j.n - len(j.select)
                if future:
                    for order in j.coming_list:
                        if order.dur == 1:  # reach in next time step
                            n_car[count] += 1
                    for k in j.select:
                        if j.orders[k].d <= config.speed_car:   # can not use d
                            # can reach in next t
